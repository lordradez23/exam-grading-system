...
export default function Login() {
  ...
  useEffect(() => {
    if (user) {
      if (user.role === "Admin" || user.role === "Faculty") router.push("/dashboard");
      ...
      <div className="p-8 border border-[var(--border)] rounded-xl bg-[var(--background)] shadow-2xl relative overflow-hidden">
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          ...
          <button
            ...
            className={`w-full py-3 mt-6 bg-[var(--foreground)] text-[var(--background)] font-semibold rounded transition-opacity ${
